  ...
  useEffect(() => {
    ...
    const handleNewMessage = (payload: any, ack?: Function) => {
      ...
      if (payload && payload.contactId === selectedContact?.id) {
        // Handle enhanced event listener format (whatsapp:message_received)
        ...
        // Handle traditional format (whatsapp:message)
    ...
    socket.off('whatsapp:message_received'); // 🚀 NEW: Enhanced event listener events
    socket.off('whatsapp:message:update');
    socket.off('room:joined');
    socket.off('room:error');

    // 🚀 CRITICAL FIX: Listen for both traditional and enhanced events
    socket.on('whatsapp:message', handleNewMessage); // Traditional events (if any)
    socket.on('whatsapp:message_received', handleNewMessage); // 🚀 NEW: Enhanced event listener events
    socket.on('whatsapp:message:update', handleMessageUpdate);

    return () => {
      socket.off('whatsapp:message', handleNewMessage);
      socket.off('whatsapp:message_received', handleNewMessage); // 🚀 NEW: Clean up enhanced events
