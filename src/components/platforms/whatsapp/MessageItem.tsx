...
const MessageItem: React.FC<MessageItemProps> = ({ 
  ...
  const isOutgoing = (() => {
    ...
    // 2. Check if we have current user info
    if (!currentUser?.id) {
      console.warn('[MessageItem] No current user available for direction detection');
      return false;
    }

    // 3. Backend logic replication: Check sender_id and sender_name for user UUID
    ...
    console.debug('[MessageItem] Direction Analysis:', {
      ...
      <div className={`flex ${isOutgoing ? 'justify-end' : 'justify-start'} ${className}`}>
        ...
        >
          ...
          <div className={`text-xs mt-1 flex ${isOutgoing ? 'justify-end' : 'justify-start'}`}>
            <span className={isOutgoing ? 'text-green-100' : 'text-gray-500'}>
              {formattedTime}
